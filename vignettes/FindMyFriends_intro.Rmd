...
organisms, together with tools to investigate the results. To the best of my
knowledge this is the first pangenomic tool in R, but also the first tool to 
rely on allignment-free analysis instead of blast all-vs-all. The number of 
tools to create pangenomes is large and some of the more notable ones are 
OrthoMCL, PanOCT, inParanoid and Sybil.
...
Metadata about the organisms can be added and queried using the `orgInfo` 
function. For instance we might add some taxonomy data.

```{r, echo=TRUE, eval=TRUE}
library(XML)
# Get uid' from accession number
entrezURL <- paste0('http://eutils.ncbi.nlm.nih.gov/entrez/eutils/',
                    'esearch.fcgi?db=nuccore&term=', 
                    orgNames(mycoPan))
uids <- sapply(entrezURL, function(url) {
    results <- xmlParse(url)
    uid <- xpathSApply(results, '//eSearchResult/IdList/Id', xmlValue)
    free(results)
    uid
})
# Get taxid from uid
entrezURL <- paste0('http://eutils.ncbi.nlm.nih.gov/entrez/eutils/',
                    'esummary.fcgi?db=nuccore&id=',
                    paste(uids, collapse=','))
results <- xmlParse(entrezURL)
taxids <- xpathSApply(results, '//eSummaryResult/DocSum/Item[@Name="TaxId"]', xmlValue)
free(results)
# Get taxonomy information
entrezURL <- paste0('http://eutils.ncbi.nlm.nih.gov/entrez/eutils/',
                    'esummary.fcgi?db=taxonomy&id=',
                    paste(taxids, collapse=','))
results <- xmlParse(entrezURL)
values <- xpathSApply(results, '//eSummaryResult/DocSum/Item', xmlValue)
types <- xpathSApply(results, '//eSummaryResult/DocSum/Item/@Name')
free(results)
taxonomy <- data.frame(
    Name=values[types=='ScientificName'],
    Genus=values[types=='Genus'],
    Species=values[types=='Species'],
    stringsAsFactors=FALSE)

head(taxonomy)

mycoPan <- addOrgInfo(mycoPan, taxonomy)
...
kmer feature vectors of the two sequences is used. Computationally this is much 
more effecient and the similarity score is neatly bounded between 0 and 1.
Furthermore cosine similarity is symmetric, as opposed to blast, and it is thus 
only necessary to calculate it once for each pair.

```{r, echo=TRUE, eval=TRUE}
mycoSim <- kmerSimilarity(mycoPan,
                          kmerSize=5,
                          lowerLimit=0.8)
```
...
mycoPan <- graphGrouping(mycoPan, mycoSim, algorithm='infomap')
mycoPan
```
...
([Aldecoa & Marín, 2013](http://www.nature.com/srep/2013/130717/srep02216/full/srep02216.html)).
...
FindMyFriends builds up a weighted graph of all genes within each group. The 
weighting is the number of shared gene groups in the neighborhood. Community 
detection is then employed to split up the resulting graphs. In the current 
iteration all gene groups flanking the group to be split is weighted equally 
(i.e. genes right next to the group does not hold more power than genes 
positioned farther away). This is done so insertions and deletions are not 
penalising the weight. The main parameter to tune is flankSize, which controls 
the number of genes up- and downstream of the gene group to include in the 
neighborhood.

```{r, echo=TRUE, eval=TRUE}
mycoPan <- neighborhoodSplit(mycoPan, flankSize=3, algorithm='infomap')
mycoPan
```

While being conceptually fairly simple this approach works both rather fast and
quite well. The only limitation is when paralogue groups are present in close
proximity (within the limit of the flankSize parameter). In these cases the 
group will not be split. This is something that might recieve attention and get
fixed in later itereations of the algorithm.
...
Usually pangenome creation is a batch process in the sense that you get the 
genomes you need, calculate the pangenome and do some analysis on it. If new 
genomes of interest becomes available you start from scratch again. Apart from 
being time consuming it also removes all knowledge on specific gene groups 
gathered over time, as there is no guarantee that gene group 1 in the first
iteration is the same as gene group 1 in the second iteration. FindMyFriends
makes it possible to gradually grow your pangenome in a manner where the 
original gene groups are preserved. It does so by effectively adding a node to
the tree used in GPC grouping with the new organism and attaching the node to 
the root of the tree. Then the GPC algorithm is just continued but with the
constraint that groups from the original set cannot be merged. This approach is 
feasible even if GPC was not used to create the initial groups.

```{r, echo=TRUE, eval=TRUE}
mycoPan <- addGenomes(mycoPan, 
                     genome=genomeFiles[10], 
                     sequenceInfo='prodigal', 
                     kmerSize=5,
                     lowerLimit=0.8,
                     flankSize=3,
                     algorithm='infomap')
newInfo <- data.frame(Name='Mycoplasma pneumoniae M129', 
                      Genus='Mycoplasma', 
                      Species='pneumoniae',
                      org='U00089')
mycoPan <- addOrgInfo(mycoPan, newInfo, key='org')
...
remove single or sets of genes from your pangenome object. By default the 
pangenome will be pruned afterwards thus removing gene groups and organisms 
without any genes left in them.
...
is supplied to either `kmerSimilarity`, `gpcGrouping`, `orgTree`, `kmerLink` or 
`addPangenome` the computations will be split out according to the supplied 
parameter. All of these, save for gpcGrouping, are embarrassingly parallel and 
thus easy to distribute evenly. gpcGrouping on the other hand is recursive and
based on an uneven tree. At the moment the paralellization works by cutting the
tree into subtrees and distributing these trees to the different workers. It is
difficult though to ensure an even distribution of computational load and 
workers will often sit idle while waiting for others to finish. A better 
implementation will maybe be investigated in the future.
